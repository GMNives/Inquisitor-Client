...
public class TrendyTheme extends Theme
{
    ...
    public boolean onRender(final Render2DEvent event) {
        if (super.onRender(event)) {
            ...
            ClientUtils.clientFont().drawStringWithShadow(String.format("§4§lInquisitor §r§a%s", time), 2.0, 2.0, -9995383);
            ClientUtils.clientFont().drawStringWithShadow(String.format("§e%s, %s, %s", Math.round(ClientUtils.x()), Math.round(ClientUtils.y()), Math.round(ClientUtils.z())), 2.0, 14.0, -9995383);
            int y = 2;
