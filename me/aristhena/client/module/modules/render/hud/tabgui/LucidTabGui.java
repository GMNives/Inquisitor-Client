...
public class LucidTabGui extends TabGui
{
    ...
    public boolean onRender(final Render2DEvent event) {
        if (super.onRender(event)) {
            ...
            RenderUtils.rectangle(3.0, LucidTabGui.categoryPosition, 2 + LucidTabGui.baseCategoryWidth - 1, LucidTabGui.categoryPosition + 14, -564655);
            int yPos = 15;
            int yPosBottom = 29;
            for (int i = 0; i < Module.Category.values().length; ++i) {
                final Module.Category category = Module.Category.values()[i];
                final String name = StringUtils.capitalize(category.name().toLowerCase());
                ClientUtils.clientFont().drawStringWithShadow(name, LucidTabGui.baseCategoryWidth / 2 - ClientUtils.clientFont().getStringWidth(name) / 2 + 3, yPos + 3, (LucidTabGui.categoryTab == i) ? -1 : -542133525);
                yPos += 14;
                yPosBottom += 14;
            }
            if (LucidTabGui.section == Section.MODS) {
                ...
                RenderUtils.rectangle(LucidTabGui.baseCategoryWidth + 5, LucidTabGui.modPosition, LucidTabGui.baseCategoryWidth + LucidTabGui.baseModWidth + 1, LucidTabGui.modPosition + 14, -564655);
                ...
                for (int i = 0; i < getModsInCategory(Module.Category.values()[LucidTabGui.categoryTab]).size(); ++i) {
                    ...
                    ClientUtils.clientFont().drawStringWithShadow(name, LucidTabGui.baseCategoryWidth + LucidTabGui.baseModWidth / 2 - ClientUtils.clientFont().getStringWidth(name) / 2 + 3, yPos + 3, (LucidTabGui.modTab == i) ? (mod.isEnabled() ? -1 : -3026479) : (mod.isEnabled() ? -564655 : -542133525));
                    yPos += 14;
                    yPosBottom += 14;
                }
            }
