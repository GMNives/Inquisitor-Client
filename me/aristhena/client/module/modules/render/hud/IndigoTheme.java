...
public class IndigoTheme extends Theme
{
    ...
    public boolean onRender(final Render2DEvent event) {
        if (super.onRender(event)) {
            String time = new SimpleDateFormat("§6HH:mm:ss").format(new Date());
            if (time.startsWith("0")) {
                time = time.replaceFirst("0", "");
            }
            GlStateManager.pushMatrix();
            GlStateManager.scale(1.7f, 1.7f, 1.7f);
            ClientUtils.clientFont().drawStringWithShadow("§4§lI", 1.0, 1.0, 7785430);
            GlStateManager.popMatrix();
            ClientUtils.clientFont().drawStringWithShadow("§4nquisitor", 9.0, 7.0, 7785430);
            ClientUtils.clientFont().drawStringWithShadow(time, 2.0, 17.0, -10063499);
            ClientUtils.clientFont().drawStringWithShadow("§6FPS: " + Minecraft.debugFPS, 2.0, 28.0, -10063499);
            int y = 2;
