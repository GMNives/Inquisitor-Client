...
public class NCP extends LongJumpModes
{
    ...
    public boolean onMove(final MoveEvent event) {
        if (super.onMove(event)) {
            if (ClientUtils.player().isCollidedHorizontally || (ClientUtils.player().moveForward == 0.0f && ClientUtils.player().moveStrafing != 0.0f)) {
                ...
                else if (NCP.stage == 1 && ClientUtils.player().isCollidedVertically && (ClientUtils.player().moveForward != 0.0f || ClientUtils.player().moveStrafing != 0.0f)) {
                ...
                if (!ClientUtils.player().isCollidedVertically && (collidingList.size() > 0 || collidingList2.size() > 0) && NCP.stage > 10) {
                    if (NCP.stage >= 38) {
