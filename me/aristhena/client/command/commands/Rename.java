...
public class Rename extends Command
{
    ...
    public void runCommand(final String[] args) {
        ...
        if (module.getId().equalsIgnoreCase("null")) {
            ClientUtils.sendMessage("Неверный модуль.");
            return;
        }
        if (newName == "") {
            ClientUtils.sendMessage(String.valueOf(module.getId()) + " имя было сброшено.");
        ...
        ModuleManager.save();
        ClientUtils.sendMessage(String.valueOf(module.getId()) + " был переименован в " + newName);
        ((Hud)new Hud().getInstance()).lucidTab.setupSizes();
    }
    
    @Override
    public String getHelp() {
        return ".rename (модуль) (название) - Переименовать модуль.";
    }
}
